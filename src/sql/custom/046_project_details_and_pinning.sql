-- Fork migration 046: richer project details (a rich-text goals/description
-- field instead of a 500-char plain string, a target end date, a
-- planning/active/on_hold/done status), plus the ability to pin a task or
-- an order so it stays at the top of its column/list regardless of sort
-- order.
...
SET @custom_exists = (
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'custom_orders' AND COLUMN_NAME = 'pinned'
);
SET @custom_sql = IF(@custom_exists = 0,
    'ALTER TABLE custom_orders ADD COLUMN pinned TINYINT(1) UNSIGNED NOT NULL DEFAULT 0',
    'SELECT 1');
PREPARE stmt FROM @custom_sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
