<script lang='ts'>
  ...
  async function loadCalendarCompletedForDate(dateKeyValue: string): Promise<void> {
    ...
    try {
      ...
      const page = await ApiC.getJson(
        `${Model.Todolist}?scope=assigned&completed=1&completed_since=${encodeURIComponent(since.toISOString())}&limit=100`,
      ) as Todo[];
  ...
  async function openFullHistory(event: MouseEvent): Promise<void> {
    ...
    const loadPage = async (): Promise<void> => {
      ...
      try {
        const page = await ApiC.getJson(
          `${Model.Todolist}?scope=assigned&completed=1&limit=${pageSize}&offset=${offset}`,
        ) as Todo[];
        page.forEach(item => {
          if (!item.completed_at) return;
  ...
  async function postComment(): Promise<void> {
    ...
    try {
      ...
      await ApiC.post(`${Model.Todolist}/${detailEntry.id}/${Model.Comment}`, { body: htmlBody, mentioned_userids: mentionedUserids });
      newCommentText = '';
      commentMentions = [];
  ...
  function mentionPoolForEntry(entry: SidebarEntry | null): TeamMember[] {
    if (!entry || entry.projectId == null) return teamMembers;
  ...
  function onCommentInput(): void {
    ...
    mentionCandidates = mentionPoolForEntry(detailEntry).filter(m => m.fullname.toLowerCase().includes(lower)).slice(0, 5);
  }
  ...
  async function saveEditComment(comment: TaskComment): Promise<void> {
    ...
    try {
      ...
      await ApiC.patch(`${Model.Todolist}/${detailEntry.id}/${Model.Comment}/${comment.id}`, { body });
      editingCommentId = null;
      await loadComments(detailEntry.id);
    } catch (error) {
  ...
  async function load(): Promise<void> {
    ...
    const [todoResponse, unfinishedResponse] = await Promise.all([
      ApiC.getJson(`${Model.Todolist}?scope=assigned&limit=${pageSize}&offset=0`) as Promise<Todo[]>,
  ...
  async function loadMore(): Promise<void> {
    ...
    const [todos, steps] = await Promise.all([
      ApiC.getJson(`${Model.Todolist}?scope=assigned&limit=${pageSize}&offset=${nextOffset}`) as Promise<Todo[]>,
        ...
        {#each selectedDateTasks as entry (entry.key)}
          <li class:todo-entry-overdue={isOverdue(entry)} class='list-group-item todo-group-entry'>
            <div class='d-flex align-items-start'>
              ...
              {#if entry.source === 'todo'}
                <div class='btn-group btn-group-sm ml-2'>
                  <button type='button' class='btn btn-ghost' on:click={() => openDetail(entry, true)} title={t('Edit')} aria-label={t('Edit')}>
                    <i class='fas fa-pen' aria-hidden='true'></i>
                  </button>
